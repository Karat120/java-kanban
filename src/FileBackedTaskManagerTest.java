...
class FileBackedTaskManagerTest {
    ...
    void shouldLoadMultipleTasksFromFile() throws IOException {
        ...
        assertAll(
                () -> assertEquals(2, loaded.getAllTasks().size(), "Должно быть загружено 2 задачи"),
                () -> assertEquals("Task A", loaded.getTaskId(1).getTaskName()),
                () -> assertEquals("Task B", loaded.getTaskId(2).getTaskName())
        );
    }
